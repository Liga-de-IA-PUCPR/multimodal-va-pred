...
class Affwild2GraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        size_of_features = len(df)

        visual_features = extract_visual_features(visual_frames_folder, num_frames_to_extract=NUM_FRAMES)
        audio_features = extract_audio_features_for_video(audio_source_path, size_of_features) # Handles non-existent path
        
        visual_features = visual_features.cpu()
        audio_features = audio_features.cpu()
        

        if visual_features.size(0) != size_of_features:
            print(f"Warning: Visual features for {video_id} have {visual_features.size(0)} frames, expected {size_of_features}. Interpolating.")
            if visual_features.size(0) == 0:
                visual_features = torch.zeros(size_of_features, VISUAL_BACKBONE_OUT_DIM)
            else:
                visual_features = visual_features.transpose(0,1).unsqueeze(0)
                visual_features = F.interpolate(
                    visual_features, size=size_of_features, mode='linear', align_corners=False
                ).squeeze(0).transpose(0,1)

        if audio_features.size(0) != size_of_features:
            print(f"Warning: Audio features for {video_id} have {audio_features.size(0)} frames, expected {size_of_features}. Interpolating.")
            if audio_features.size(0) == 0:
                audio_features = torch.zeros(size_of_features, AUDIO_MEL_N_MELS)
            else:
                audio_features = audio_features.transpose(0, 1).unsqueeze(0)
                audio_features = F.interpolate(
                    audio_features, size=size_of_features, mode='linear', align_corners=False
                ).squeeze(0).transpose(0, 1)
