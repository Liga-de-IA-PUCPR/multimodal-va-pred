...
def build_video_graph(visual_features: torch.Tensor, audio_features: torch.Tensor) -> Data:
    """
    Construct a graph for a single video, representing frames as nodes
    and temporal connections as edges.
    
    Args:
        visual_features (torch.Tensor): Tensor of shape [num_frames, visual_dim].
                                        Expected on CPU.
        audio_features (torch.Tensor): Tensor of shape [num_frames, audio_dim].
                                       Expected on CPU.
                                       
    Returns:
        torch_geometric.data.Data: A PyG Data object representing the video graph.
                                   Node features are a concatenation of visual and audio features.
                                   All tensors in the Data object are on CPU.
    """
    num_frames_visual = visual_features.size(0)
    num_frames_audio = audio_features.size(0)
    
    if num_frames_visual != num_frames_audio:
        raise ValueError(
            f"Mismatch in frame count: Visual features have {num_frames_visual} frames, "
            f"audio features have {num_frames_audio} frames. They must be aligned before graph construction."
        )
    ...
    if num_frames == 0:
        ...
        combined_feature_dim = visual_features.size(1) + audio_features.size(1) if visual_features.ndim > 1 and audio_features.ndim > 1 else 0
        return Data(x=torch.empty((0, combined_feature_dim), dtype=torch.float), 
                    edge_index=torch.empty((2,0), dtype=torch.long))

    node_features = torch.cat([visual_features, audio_features], dim=1)
